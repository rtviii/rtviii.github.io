...
- §2.1.1 **Hierarchy** -- the physical tree: which atom is in which residue, which residue is in which chain, and so on.
- §2.1.2 **Representation** -- named DAGs over hierarchy: domain partitions, coarse-grained mappings, rigid-body groups, multiscale pyramids.
- §2.2.1 **Heterogeneity** -- how states vary, classified into three regimes with scopes attached to hierarchy levels.
- §2.2.2 **Materialization** -- how those descriptions turn into actual coordinates.
...
- The **representation DAG** (§2.1.2) is a mapping from hierarchy nodes to named groupings: atoms to coarse-grained beads, residues to domains, residues to TLS groups. Sparse bipartite, weighted, many per format.
- The **scope DAG** (§2.2.1) is a state-space structure: nodes are (scope, state) pairs, edges are nesting or coupling constraints between heterogeneity descriptors at different hierarchy levels.
- The **operator graph** (§2.2.2) is implicit in Materialization Mode C: a decoder or mode basis is a stored operator whose inputs are latent coordinates and whose outputs are displacement fields.
