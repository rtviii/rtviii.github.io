...
![](data/figures/fig-00-overlay-bus.png){fig-alt="Annotation overlay attaches to a MolQL-style selector bus, which addresses subsets of the Hierarchy and Groupings core; the four grouping panels show parallel typed namespaces over the same atom cloud."}

### 2.1.1 Hierarchy {#containment}

![](data/figures/fig-01-containment.png){fig-alt="Five-level mmCIF containment tree: Assembly contains Entities (polymer, ligand, water), each containing Chains, Residues, and finally atoms."}
...
![](data/figures/fig-02-typed-groupings.png){fig-alt="Three typed groupings (curatorial CATH, coarse-grain bead, het-scope TLS) over the same Chain A residues; each is a sparse bipartite mapping plus a kind tag, with the het-scope grouping additionally bound to a registry-referenced operator."}
...
![](data/figures/fig-03-regime1-discrete.png){fig-alt="Regime 1: a residue with three discrete states, each carrying its own per-atom coordinates and a population fraction."}
...
![](data/figures/fig-04-regime2-trajectory.png){fig-alt="Regime 2: a residue's atoms with the CA coordinate as a dense per-frame array along an explicit frame axis; per-frame scalars (time, energy, box) ride alongside."}
...
![](data/figures/fig-05-regime3-continuous.png){fig-alt="Regime 3: a low-dimensional latent z with a stored decoder phi that maps z to per-atom coordinates; named landmarks appear as clusters in z-space rather than as fundamental objects."}
...
![](data/figures/fig-06-materialization-modes.png){fig-alt="Three storage strategies for the coordinate leaf: Mode A enumerates every state, Mode B stores a base plus a sparse delta table, Mode C stores a compact input plus an operator that produces coordinates on demand."}
