...
::: {.content-hidden}
## Working notes — forward operators, the evaluation model, and the ML-on-primary-data case {#wip-notes .unlisted .unnumbered}

*Scratch synthesis, not rendered (`content-hidden`). To be developed into prose later; the citation keys are already wired into the global numbering, so they resolve the moment any of this is promoted to visible text.*

**Forward operators are the dual of the evaluation model, not a separate stack.** The evaluation model composes *inward*: descriptors, latents, and operator references render down to coordinates (Materialization Modes A/B/C). A forward operator composes *outward*: it takes those coordinates / the ensemble and predicts what an instrument would have measured; with a noise model that prediction becomes a likelihood. The two chain directly — the evaluation model's output is the forward operator's input — so end-to-end and differentiable they are one graph: latent $z \to$ decoder $\to$ coordinates $\to$ forward operator $\to$ predicted observable $\to$ noise model $\to$ likelihood, gradients flowing all the way back. They run on the *same machinery*: the registry, content-hashing and versioning, the interface-contract discipline, the three storage venues (inline / registry-referenced / external), the differentiability convention. Hence the note in §1 that a consumer implementing the Mode C runtime is most of the way to forward operators. Slogan: the evaluation model *materializes* the ensemble; the forward operators *measure* it. Same socket, opposite direction.

**"Capturing multiple fields" is bounded once you see what is and isn't unified.** The format does not unify raw primary-data shapes — diffraction images, FIDs, micrographs, scattering curves stay native and heterogeneous (§1). What is unified is the atomic ensemble, plus a per-technique operator interface declaring what the operator consumes from the ensemble and what observable it emits. Adding a field is therefore additive: one operator interface and a noise model, no change to the core. That is the whole answer to the "capture all things" worry — we standardize one socket, we do not subsume crystallography or cryo-EM or NMR. The hard part is sociological, not technical: agreeing on a canonical `structure_factor_v1`, `cryo_em_map_v1`, `noe_v1` (§3). And you do not need that consensus to start — a novel operator ships today as a content-hashed differentiable blob (embedded graph); canonicalization happens later if the community converges. So the realistic near-term scope is exactly the set of fields whose forward operator someone has already written as a differentiable function — precisely the frontier the ML-guidance work already stands on.

**Why now: the ML-on-primary-data wave is independently converging on this abstraction.** The trend is methods going back to the primary data to recover the heterogeneity the single-model pipeline discards — the standing ~20% R-free residual is the evidence that one static structure under-explains its own data. A cluster of 2025–26 work guides pretrained predictors to fit experimental data at inference time: ADP-3D [@levy2025inverse] (diffusion prior plus explicit cryo-EM / distance forward operators), CryoBoltz [@raghu2025cryoboltz] (Boltz [@passaro2025boltz] guided by cryo-EM density), CrystalBoltz [@kim2026crystalboltz] (Boltz-2 guided by crystallographic structure factors), EmbedOpt [@li2026embedopt] (Protenix [@protenix2025advancing] embeddings steered by density / NMR restraints), ROCKET [@fadini2026rocket] (AlphaFold-as-prior conditioned on cryo-EM and crystallographic data). Each needs the same two things: a differentiable forward operator from ensemble to observable, and a representation that holds an ensemble rather than a single model — Track B and Track A.

sampleworks [@chrispens2026sampleworks] is the cleanest evidence the abstraction is real, because it is the framework-level admission of exactly this fragmentation. Four primitives: ModelWrappers (the predictor — Boltz-2 [@passaro2025boltz], Protenix [@protenix2025advancing], RoseTTAFold3 [@corley2025rf3]), LossFunctions (a differentiable map from a predicted ensemble to agreement-with-data — *this is a forward operator plus comparison*), Samplers, Scalers (the guidance loop — diffusion posterior sampling [@chung2024dps], Feynman–Kac steering [@singhal2025fk]) — mix and match. Its stated motivation is verbatim the Track B argument: existing guidance methods are "bespoke to specific structural predictors," each "tests on non-overlapping sets," integrating them "carries substantial engineering overhead." Its first benchmark (40 high-res X-ray entries, 791 altloc segments, guided by synthetic noise-free density) finds predictors stay biased to training-set conformations and density guidance helps but does not reliably recover both altlocs — i.e. the ensemble problem is open and needs this kind of infrastructure to make progress measurable.

The gap sampleworks leaves — the one the registry fills — is persistence and portability. sampleworks standardizes the software interface *within one framework*; the forward operator (its LossFunction) is still re-specified per framework and does not travel with the deposited data. Make the forward operator a versioned, content-hashed artifact (`cryo_em_map_v1@1.0.0`) referenced from the deposited ensemble and the same loss means the same number across sampleworks, the next framework, and a benchmark three years out; the deposited ensemble carries the operator and the observation it was fit against; re-fitting and cross-method comparison become mechanical rather than a per-paper engineering project. The pitch: turn experimentally-guided ensemble generation from a research codebase into deployable infrastructure, and give the next generation of predictors ensembles-with-their-evidence as training targets instead of lossy single structures.

*Ambition without arrogance:* not "one format for all of structural biology's data," but "the forward operator deserves to be a persistent, shareable artifact rather than per-project code" — a pattern practitioners (sampleworks most explicitly) are already rebuilding by hand. Standardize the socket; let the fields keep their data.
:::
