...
- Where is the Regime 3/4 boundary enforced? In the spec (a validator rejects factor graphs over more than N variables), in tooling (the writer library warns and suggests a mode basis), or only in convention?
- Does the bond graph belong in the containment core? The argument for: bond connectivity is not reliably inferrable for arbitrary ligands. The argument against: it is derivable for all standard residues via CCD lookup, and putting it in the core creates a maintenance burden for the bond dictionary. Resolution probably requires examining what Boltz-1 and AF3 actually do when they encounter an unknown ligand.
