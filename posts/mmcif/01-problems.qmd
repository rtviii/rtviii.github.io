...
- DeepMind's AlphaFold rolls its own (non-biopython/gemmi/etc) mmCIF parser: [huggingface.co/.../mmcif_parsing.py](https://huggingface.co/spaces/simonduerr/ProteinMPNN/blob/f969e9cfb6f11ba299c7108aadff124e9cf38b1f/alphafold/alphafold/data/mmcif_parsing.py)
- model-angelo (GNN model building) parsing issue: [github.com/3dem/model-angelo/issues/51](https://github.com/3dem/model-angelo/issues/51)
...
- Biopython developer complaints: [stackoverflow.com/a/11686524](https://stackoverflow.com/a/11686524)
- AF issue: [github.com/google-deepmind/alphafold/issues/252](https://github.com/google-deepmind/alphafold/issues/252)
...
::: {.fold data-summary="Some cif specs"}
...
The legacy PDB <-> mmCIF round-trip silently mangles information. Chain IDs longer than one character get truncated. The 99,999-atom limit forces hex hacks. Residue numbers above 9999 break. Insertion codes vanish in some pipelines. Encoding and line-ending bugs cause half-loaded structures whose symptoms surface downstream in the viewer, not at parse time. gemmi [@wojdyr2022gemmi] is the strictest modern mmCIF library and flags edge cases other parsers swallow.

## Content-level errors that survive deposition

Even files that parse cleanly carry content-level errors that wwPDB validation does not catch. PDB-REDO [@joosten2014pdbredo] systematically re-refines the X-ray archive and finds measurable improvements often enough to be a standing critique of the deposited model. Recurring categories:

- Missing or extra atoms per residue; atom names that do not match the CCD; modified residues mis-typed.
- Geometric outliers: bond length, angle, clash, cis/trans peptide, chirality. MolProbity [@williams2018molprobity] for protein geometry; Mogul for ligand geometry.
...
- Composition errors: wrong CCD ligand identifier; protonation/tautomer guesses baked in silently; Mg^2+^ modelled as water at low resolution. CheckMyMetal and CheckMyBlob are dedicated checks.
...
- **MolProbity** [@williams2018molprobity] -- Ramachandran, rotamer, clash, geometry Z-scores.
- **Coot** [@emsley2010coot] -- interactive density-and-geometry validation during refinement.
- **Phenix** [@liebschner2019phenix] -- `phenix.molprobity`, `phenix.validate_geometry`, `phenix.real_space_correlation`.
...
- **gemmi** [@wojdyr2022gemmi] -- parser-level diagnostics.
- **PDB-REDO** [@joosten2014pdbredo] -- systematic re-refinement of the X-ray archive.
...
- **ChimeraX** [@pettersen2021chimerax] -- bundled validation panels and a Python API for custom checks.
...
::: {.fold data-summary="Some extensions in important directions"}

**BinaryCIF** ([github.com/molstar/BinaryCIF](https://github.com/molstar/BinaryCIF)) addresses the performance problem: binary-encoded mmCIF with column-wise compression, significantly smaller files and faster parsing than text mmCIF. Used by the PDB for Mol* visualization. But it's a serialization optimization, not a data model change -- it encodes the same categories and the same single-structure assumption.

The **IHMCIF dictionary** [@ihmcif] (the IHM working group extension). This adds: discrete multi-state models, ordered ensembles (states connected by time or other ordering), multi-scale representations (atomic + coarse-grained beads in the same model), and spatial restraint descriptions.  But: states remain discrete and independent, there is no thermodynamic annotation (or other flexible heterogeneity annotations either), no per-state uncertainty, no support for continuous distributions, and the text-file serialization doesn't scale to large ensembles.
