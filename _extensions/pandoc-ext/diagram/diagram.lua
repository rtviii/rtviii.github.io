...
local tikz = {
  ...
  compile = function (self, src, user_opts)
    -- Local patch: read the `preamble-file:` BEFORE switching into the
    -- temp directory; the path is resolved against the cwd we entered
    -- with (Quarto runs the filter from the project root). Pandoc's
    -- `stringify` collapses multi-line YAML block scalars to empty for
    -- LaTeX content, so the file route is the reliable way to inject
    -- a real preamble.
    local extra_preamble = stringify(user_opts['additional-packages'] or '')
    local preamble_file = stringify(user_opts['preamble-file'] or '')
    if preamble_file ~= '' then
      if not pandoc.path.is_absolute(preamble_file) then
        preamble_file = pandoc.path.join{
          pandoc.system.get_working_directory(),
          preamble_file
        }
      end
      local fh = io.open(preamble_file, 'rb')
      if fh then
        extra_preamble = extra_preamble .. '\n' .. fh:read('a')
        fh:close()
      else
        warn('diagram: preamble-file not found: ' .. preamble_file)
      end
    end
...
-- Local patch: upstream uses inkscape; we use dvisvgm (TeX Live ships it,
-- avoids the inkscape dependency). dvisvgm writes to a file, so we read
-- it back rather than capturing stdout.
local pdf2svg = function (imgdata)
  local hash = pandoc.utils.sha1(imgdata)
  local pdf_file = 'diagram-' .. hash .. '.pdf'
  local svg_file = 'diagram-' .. hash .. '.svg'
  write_file(pdf_file, imgdata)
  -- `--font-format=woff2` keeps text as <text> elements with embedded
  -- WOFF2 fonts; this preserves Unicode, math, and superscripts/
  -- subscripts that --no-fonts (paths) sometimes drops on subtle glyphs.
  pandoc.pipe(
    'dvisvgm',
    { '--pdf', '--font-format=woff2', '--output=' .. svg_file, pdf_file },
    ''
  )
